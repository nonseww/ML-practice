...
```
1. Делаем bootstrap-выборки
2. Обучаем отдельную модель на каждой
3. Объединяем предсказания
```
...
- max_features (сколько признаков рассматривать при поиске split)

## 12. Gradient Boosting - градиентный бустинг

> Gradient Boosting строит деревья последовательно, и каждое новое дерево пытается исправить ошибки уже построенного ансамбля.

Мы юзаем градиент функции потерь.

### Гиперпараметры

- learning_rate (насколько сильно каждое новое дерево влияет на итоговый результат) -> маленький он, маленький шаг, и наоборот
- n_estimators (количество построенных деревьев)
- max_depth (максимальная глубина дерева)

## 13. CatBoost

Это алгоритм градиентного бустинга на деревьях.

> Он умеет работать с категориальными признаками специальным образом, поэтому часто не нужно вручную делать One-Hot Encoding для всех категорий.

### Target Encoding

Пусть мы хотим предсказать:
...
Посчитаем средний target для каждого города.
...
То есть категория заменяется некоторой статистикой target.

### CatBoost's alhgo

Он имеет специальные механизмы обработки категорий, не это. Причем он делает это так, чтобы **сильно уменьшить риск leakage**, который возникает при обычном target encoding.

|                         | Random Forest                  | CatBoost                     |
| ----------------------- | ------------------------------ | ---------------------------- |
| Тип                     | Bagging                        | Boosting                     |
| Деревья                 | независимые                    | последовательные             |
| Основная идея           | усреднить много деревьев       | постепенно исправлять ошибки |
| Категориальные признаки | сам по себе не решает проблему | умеет работать с ними        |
| Масштабирование         | обычно не нужно                | обычно не нужно              |
| Глубокие деревья        | могут использоваться           | обычно деревья неглубокие    |

## 14. KNN - K ближайших соседей

> Чтобы предсказать новый объект, посмотрим на его K ближайших соседей в пространстве признаков.
...
> SVM ищет границу, которая разделяет классы с **максимально возможным зазором (margin)**.

Ближайшие к границе объекты - это опорные векторы.

### Kernel trick

Что если классы нельзя разделить прямой линией?

> Мы можем перейти в пространство признаков, где классы можно разделить более простой границей.

### Гиперпараметры

- C (насколько сильно модель штрафует ошибки на обучающих данных)
- gamma (для RBG kernel. Насколько ломанной может быть граница)

## 16. K-means

Кластеризация, unsuperviced learning. Пытаемся разбить по кластерам. Выбирает центроиды, каждый объект относим к ближайшему центроиду, потом пересчитываем центры и повторить.

> K-means старается сделать маленьким расстояние от объектов до центров своих кластеров.

Обычно юзают **квадрат евклидова расстояния**.
...
Мы строим зависимость:

`K -> ошибка внутри кластеров`

![](./images/Elbow-Method.png)

### Метрика: Silhoutte Score

> Оценивает, насколько объект 1. близок к объектам своего кластера; 2. далек от объектов других кластеров

```
≈ 1    → хорошо
≈ 0    → кластеры сильно пересекаются
< 0    → объект может быть ближе к другому кластеру
```

![](./images/clustering.jpg)

## 17. PCA - Principal Component Analysis (метод главных компонент)

Это алгоритм **снижения размерности**.

Признаков может быть много, но они моугт быть связаны между собой. РСА пытается заменить множество исходных признаков мнеьшим количеством новых признаков - главных компонент.

> PCA пытается найти направления, в котором данные имеют **максимальную дисперсию** -> первая главная компонента - направление максимальной вариативности данных.

> Вторая компонента - следующее направление, которое 1. объясняет как можно больше оставшейся дисперсии; 2. ортогонально первой компоненте.
...
```
PC1 + PC2
= 90% исходной дисперсии
```

Можно оставить две компоненты вместо десятков признаков и сохранить большую часть вариативности данных.

> важно масштабировать

## 18. Naive Bayes

Это **алгоритм классификации**, основанный на теореме Байеса.

> Идея: если я вижу такие признаки, к какому классу это наблюдение вероятнее всего относится?

Пусть:

хотим определить, является ли письмо спамом.
...
- есть ссылка;
- письмо отправлено неизвестным адресом.

Алгоритм оценивает вероятность:

**P(спам | признаки)**

и сравнивает её с вероятностью:

**P(не спам | признаки)**

> Признаки считаются условно независимыми друг от друга при фиксированном классе.

## 19. DBSCAN

это **алгоритм кластеризации**.

> Идея: найди области, где точки расположены достаточно плотно.

Это отличает его от K-means. Мы не задаем заранее количество кластеров, которые надо найти, алгоритм сам их ищет.

### Гиперпараметры

- eps (радиус окрестности точки) - насколько близко должна находиться другая точка, чтобы считаться её соседом?
- min_samples (минимальное количество точек в окрестности, чтобы считать область достаточно плотной)

> имеет находить кластеры сложной формы
