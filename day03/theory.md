...
```python
df.iloc[номер_строки, номер_столбца] # общий вид
...
## 13. Дубликаты ключей merge

Если ключ встречается:

- `m` раз в левой таблице
- `n` раз в правой

то получится `m * n` строк для этого ключа (**mane-to-many merge**).

## 14. Типы связей при merge
...
## 15. NaN и пропущенные значения

`NaN` это пропущенное значение.

```python
df.isna() # проверить пропуски
# получим таблицу True/False (True - значения нет)
...
df["age"] = df["age"].fillna(0) # на 0
df["age"] = df["age"].fillna(df["age"].mean()) # на среднее
```

## 16. Leakage при заполнении пропусков

**Статистики для preprocessing нужно вычислять только на train**.
...
`concat()` объединяет (склеивает) таблицы.
