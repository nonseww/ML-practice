...
Но почему у `90` теперь `3`, а не `2`? Потому что два человека разделили первое место.

|                     | `ROW_NUMBER()` | `RANK()`        |
| ------------------- | -------------- | --------------- |
| Одинаковые значения | разные номера  | одинаковый ранг |
| Есть пропуски       | нет            | да              |
| 100, 100, 90        | 1, 2, 3        | 1, 1, 3         |

## 23. DENSE_RANK()

Работает как `RANK()`, но не оставляет пропусков.
